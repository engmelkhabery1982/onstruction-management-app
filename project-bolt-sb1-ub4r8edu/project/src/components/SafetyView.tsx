import { useMemo, useRef, useState } from 'react';
import { Upload, Download, ShieldAlert, Loader2 } from 'lucide-react';
import { SpreadsheetGrid, type Column } from '@/components/SpreadsheetGrid';
import type { Safety, Project } from '@/types';
import { SAFETY_STATUSES, SAFETY_TYPES, SAFETY_SEVERITIES } from '@/types';
import { exportSafetyToExcel, parseSafetyExcel } from '@/utils/excel';

interface SafetyViewProps {
  safety: Safety[];
  projects: Project[];
  onCellChange: (id: string, key: string, value: string | number) => void;
  onAddRow: () => void;
  onDeleteRow: (id: string) => void;
  onBulkImport: (rows: Partial<Safety>[]) => Promise<void>;
}

const severityColors: Record<string, string> = {
  Low: 'text-success-600 bg-success-50',
  Medium: 'text-warning-600 bg-warning-50',
  High: 'text-accent-600 bg-accent-50',
  Critical: 'text-error-600 bg-error-50',
};

export function SafetyView({ safety, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: SafetyViewProps) {
  const fileRef = useRef<HTMLInputElement>(null);
  const [importing, setImporting] = useState(false);
  const [filterProject, setFilterProject] = useState('all');
  const [filterStatus, setFilterStatus] = useState('all');

  const projectNameMap = useMemo(() => {
    const map: Record<string, string> = {};
    projects.forEach((p) => { map[p.id] = p.name; });
    return map;
  }, [projects]);

  const projectOptions = useMemo(() => projects.map((p) => p.name), [projects]);
  const projectNameToId = useMemo(() => {
    const map: Record<string, string> = {};
    projects.forEach((p) => { map[p.name] = p.id; });
    return map;
  }, [projects]);

  const columns: Column<Safety>[] = useMemo(() => [
    {
      key: 'project_id', header: 'Project', width: 200, type: 'select', options: projectOptions,
      format: (v) => projectNameMap[v as string] || '—',
    },
    { key: 'type', header: 'Type', width: 120, type: 'select', options: SAFETY_TYPES },
    {
      key: 'severity', header: 'Severity', width: 100, type: 'select', options: SAFETY_SEVERITIES,
      format: (v) => {
        const s = String(v);
        const cls = severityColors[s] || '';
        return s;
      },
    },
    { key: 'date', header: 'Date', width: 120, type: 'date' },
    { key: 'description', header: 'Description', width: 280, type: 'text' },
    { key: 'location', header: 'Location', width: 140, type: 'text' },
    { key: 'responsible', header: 'Responsible', width: 140, type: 'text' },
    { key: 'status', header: 'Status', width: 120, type: 'select', options: SAFETY_STATUSES },
    { key: 'action_taken', header: 'Action Taken', width: 280, type: 'text' },
  ], [projectOptions, projectNameMap]);

  const filtered = safety.filter((s) => {
    if (filterProject !== 'all' && s.project_id !== filterProject) return false;
    if (filterStatus !== 'all' && s.status !== filterStatus) return false;
    return true;
  });

  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setImporting(true);
    try {
      const rows = await parseSafetyExcel(file);
      const mapped = rows.map((r) => ({
        ...r,
        project_id: filterProject !== 'all' ? filterProject : (projects[0]?.id || ''),
      }));
      await onBulkImport(mapped);
    } catch (err) {
      alert('Import failed: ' + (err instanceof Error ? err.message : 'Unknown error'));
    } finally {
      setImporting(false);
      if (fileRef.current) fileRef.current.value = '';
    }
  };

  const handleCellChange = (id: string, key: string, value: string | number) => {
    if (key === 'project_id') {
      const pid = projectNameToId[value as string] || value;
      onCellChange(id, 'project_id', pid);
      return;
    }
    onCellChange(id, key, value);
  };

  const openCount = filtered.filter((s) => s.status === 'Open').length;
  const criticalCount = filtered.filter((s) => s.severity === 'Critical' || s.severity === 'High').length;

  return (
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      <div className="border-b border-neutral-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <ShieldAlert size={20} className="text-error-600" />
          <h2 className="text-base font-semibold text-neutral-800">Safety & HSE</h2>
          <span className="text-xs text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded-full">{filtered.length}</span>
        </div>

        <div className="flex items-center gap-4 ml-4">
          {openCount > 0 && (
            <div className="text-xs text-error-600 font-medium bg-error-50 px-2 py-1 rounded-lg">
              {openCount} open
            </div>
          )}
          {criticalCount > 0 && (
            <div className="text-xs text-accent-600 font-medium bg-accent-50 px-2 py-1 rounded-lg">
              {criticalCount} high/critical
            </div>
          )}
        </div>

        <div className="flex-1" />

        <select
          value={filterProject}
          onChange={(e) => setFilterProject(e.target.value)}
          className="text-sm px-3 py-1.5 border border-neutral-200 rounded-lg bg-white focus:outline-none focus:border-primary-400 max-w-48"
        >
          <option value="all">All Projects</option>
          {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>

        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value)}
          className="text-sm px-3 py-1.5 border border-neutral-200 rounded-lg bg-white focus:outline-none focus:border-primary-400"
        >
          <option value="all">All Statuses</option>
          {SAFETY_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>

        <div className="h-6 w-px bg-neutral-200" />

        <label className="text-sm px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg hover:bg-primary-100 flex items-center gap-1.5 cursor-pointer transition-colors border border-primary-200">
          {importing ? <Loader2 size={15} className="animate-spin" /> : <Upload size={15} />}
          Import Excel
          <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleImport} className="hidden" />
        </label>

        <button
          onClick={() => exportSafetyToExcel(filtered)}
          disabled={filtered.length === 0}
          className="text-sm px-3 py-1.5 bg-success-50 text-success-700 rounded-lg hover:bg-success-100 flex items-center gap-1.5 transition-colors border border-success-200 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <Download size={15} /> Export Excel
        </button>
      </div>

      <div className="flex-1 overflow-hidden">
        <SpreadsheetGrid<Safety>
          columns={columns}
          rows={filtered}
          onCellChange={handleCellChange}
          onDeleteRow={onDeleteRow}
          onAddRow={onAddRow}
          getRowId={(s) => s.id}
          emptyMessage="No safety records yet. Click 'Add Row' or import from Excel to get started."
        />
      </div>
    </div>
  );
}
