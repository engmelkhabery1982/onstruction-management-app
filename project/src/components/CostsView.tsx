...
import { Upload, Download, DollarSign, Loader as Loader2, Search } from 'lucide-react';
...
export function CostsView({ costs, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: CostsViewProps) {
  ...
  const filtered = costs.filter((c) => {
    ...
    if (search && !c.description.toLowerCase().includes(search.toLowerCase()) && !c.notes.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
