...
import { Upload, Download, ClipboardList, Loader as Loader2, Search } from 'lucide-react';
...
import { exportBOQToExcel, parseBOQExcel } from '@/utils/excel';
...
interface BOQViewProps {
  ...
  onBulkImport: (rows: Partial<BOQItem>[]) => Promise<void>;
}
...
export function BOQView({ boqItems, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: BOQViewProps) {
  ...
  const columns: Column<BOQItem>[] = useMemo(() => [
    { key: 'project_id', header: 'Project', width: 180, type: 'select', options: projectOptions, format: (v) => projectNameMap[v as string] || '—' },
    { key: 'item_code', header: 'Item Code', width: 120, type: 'text' },
    { key: 'description', header: 'Description', width: 300, type: 'text' },
    { key: 'category', header: 'Category', width: 140, type: 'select', options: BOQ_CATEGORIES },
    { key: 'unit', header: 'Unit', width: 80, type: 'text' },
    { key: 'quantity', header: 'Qty', width: 100, type: 'number', align: 'right' },
    { key: 'unit_rate', header: 'Rate', width: 120, type: 'number', align: 'right', format: (v) => fmtMoney(v) },
    { key: 'amount', header: 'Amount', width: 140, type: 'number', align: 'right', format: (v) => fmtMoney(v) },
    { key: 'notes', header: 'Notes', width: 200, type: 'text' },
  ], [projectOptions, projectNameMap]);

  const filtered = boqItems.filter((b) => {
    if (filterProject !== 'all' && b.project_id !== filterProject) return false;
    if (filterCategory !== 'all' && b.category !== filterCategory) return false;
    if (search && !b.description.toLowerCase().includes(search.toLowerCase()) && !b.item_code.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      <div className="border-b border-neutral-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-neutral-800">Bill of Quantities</h2>
        ...
        <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="text-sm px-3 py-1.5 border border-neutral-200 rounded-lg bg-white focus:outline-none focus:border-primary-400">
          <option value="all">All Categories</option>
          {BOQ_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <div className="relative">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none" />
          <input type="text" placeholder="Search description, code..." value={search} onChange={(e) => setSearch(e.target.value)} className="text-sm pl-9 pr-3 py-1.5 border border-neutral-200 rounded-lg w-48 focus:outline-none focus:border-primary-400" />
        </div>
        ...
        <button onClick={() => exportBOQToExcel(filtered, projectNameMap)} disabled={filtered.length === 0} className="text-sm px-3 py-1.5 bg-success-50 text-success-700 rounded-lg hover:bg-success-100 flex items-center gap-1.5 transition-colors border border-success-200 disabled:opacity-40 disabled:cursor-not-allowed">
          <Download size={15} /> Export Excel
        </button>
      </div>
      <div className="flex-1 overflow-hidden">
        <SpreadsheetGrid<BOQItem> columns={columns} rows={filtered} onCellChange={handleCellChange} onDeleteRow={onDeleteRow} onAddRow={onAddRow} getRowId={(b) => b.id} emptyMessage="No BOQ items yet. Click 'Add Row' or import from Excel." />
      </div>
    </div>
  )
  );
