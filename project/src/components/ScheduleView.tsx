...
import { Upload, Download, CalendarClock, Loader as Loader2, Search } from 'lucide-react';
...
export function ScheduleView({ schedules, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: ScheduleViewProps) {
  ...
  const filtered = schedules.filter((s) => {
    ...
    if (search && !s.activity.toLowerCase().includes(search.toLowerCase()) && !s.responsible.toLowerCase().includes(search.toLowerCase()) && !s.notes.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      <div className="border-b border-neutral-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search activity..." value={search} onChange={(e) => setSearch(e.target.value)} className="text-sm pl-9 pr-3 py-1.5 border border-neutral-200 rounded-lg w-48 focus:outline-none focus:border-primary-400" />
        </div>
