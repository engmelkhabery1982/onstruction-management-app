...
import { exportDocumentsToExcel, parseDocumentsExcel } from '@/utils/excel';
...
interface DocumentsViewProps {
  ...
  onBulkImport: (rows: Partial<DocumentEntry>[]) => Promise<void>;
}

export function DocumentsView({ documents, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: DocumentsViewProps) {
  ...
  const columns: Column<DocumentEntry>[] = useMemo(() => [
    { key: 'project_id', header: 'Project', width: 180, type: 'select', options: projectOptions, format: (v) => projectNameMap[v as string] || '—' },
    { key: 'document_name', header: 'Document Name', width: 220, type: 'text' },
    { key: 'document_type', header: 'Type', width: 140, type: 'select', options: DOCUMENT_TYPES },
    { key: 'category', header: 'Category', width: 140, type: 'text' },
    { key: 'version', header: 'Version', width: 90, type: 'text' },
    { key: 'upload_date', header: 'Upload Date', width: 130, type: 'date' },
    { key: 'status', header: 'Status', width: 120, type: 'select', options: DOCUMENT_STATUSES },
    { key: 'responsible', header: 'Responsible', width: 150, type: 'text' },
    { key: 'file_reference', header: 'File Reference', width: 180, type: 'text' },
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try {
      const rows = await parseDocumentsExcel(file);
  ...
  const currentCount = filtered.filter((d) => d.status === 'Current').length;
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      <div className="border-b border-neutral-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-neutral-800">Documents</h2>
          <span className="text-xs text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded-full">{filtered.length}</span>
        </div>
        <div className="flex items-center gap-4 ml-4">
          <div className="text-xs text-neutral-500">
            <span className="text-neutral-400">Current:</span> <span className="font-semibold text-primary-600">{currentCount}</span>
          </div>
          <div className="text-xs text-neutral-500">
        ...
        <button onClick={() => exportDocumentsToExcel(filtered, projectNameMap)} disabled={filtered.length === 0} className="text-sm px-3 py-1.5 bg-success-50 text-success-700 rounded-lg hover:bg-success-100 flex items-center gap-1.5 transition-colors border border-success-200 disabled:opacity-40 disabled:cursor-not-allowed">
