...
import { Upload, Download, FileText, Loader as Loader2, Search } from 'lucide-react';
import { SpreadsheetGrid, type Column } from '@/components/SpreadsheetGrid';
import type { ClientInvoice, Project } from '@/types';
import { CLIENT_INVOICE_STATUSES, PAYMENT_STATUSES } from '@/types';
import { exportClientInvoicesToExcel, parseClientInvoicesExcel } from '@/utils/excel';
...
interface ClientInvoicesViewProps {
  ...
  onBulkImport: (rows: Partial<ClientInvoice>[]) => Promise<void>;
}
...
export function ClientInvoicesView({ clientInvoices, projects, onCellChange, onAddRow, onDeleteRow, onBulkImport }: ClientInvoicesViewProps) {
  ...
  const columns: Column<ClientInvoice>[] = useMemo(() => [
    ...
    { key: 'due_date', header: 'Due Date', width: 130, type: 'date' },
    { key: 'amount', header: 'Amount', width: 130, type: 'number', align: 'right', format: (v) => fmtMoney(v) },
    { key: 'status', header: 'Status', width: 120, type: 'select', options: CLIENT_INVOICE_STATUSES },
  ...
  const filtered = clientInvoices.filter((c) => {
    if (filterProject !== 'all' && c.project_id !== filterProject) return false;
    if (filterStatus !== 'all' && c.status !== filterStatus) return false;
    if (search && !c.client.toLowerCase().includes(search.toLowerCase()) && !c.invoice_number.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try {
      const rows = await parseClientInvoicesExcel(file);
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      <div className="border-b border-neutral-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-neutral-800">Client Invoices</h2>
          <span className="text-xs text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded-full">{filtered.length}</span>
        </div>
        <div className="flex items-center gap-4 ml-4">
          <div className="text-xs text-neutral-500">
            <span className="text-neutral-400">Invoiced:</span> <span className="font-semibold text-neutral-700">{fmtMoney(totalAmount)}</span>
          </div>
          <div className="text-xs text-neutral-500">
            <span className="text-neutral-400">Received:</span> <span className="font-semibold text-success-600">{fmtMoney(totalPaid)}</span>
          </div>
          <div className="text-xs text-neutral-500">
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-sm px-3 py-1.5 border border-neutral-200 rounded-lg bg-white focus:outline-none focus:border-primary-400">
          ...
          {CLIENT_INVOICE_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <div className="relative">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none" />
          <input type="text" placeholder="Search client..." value={search} onChange={(e) => setSearch(e.target.value)} className="text-sm pl-9 pr-3 py-1.5 border border-neutral-200 rounded-lg w-48 focus:outline-none focus:border-primary-400" />
        </div>
        ...
        <button onClick={() => exportClientInvoicesToExcel(filtered, projectNameMap)} disabled={filtered.length === 0} className="text-sm px-3 py-1.5 bg-success-50 text-success-700 rounded-lg hover:bg-success-100 flex items-center gap-1.5 transition-colors border border-success-200 disabled:opacity-40 disabled:cursor-not-allowed">
          <Download size={15} /> Export Excel
        </button>
      </div>
      <div className="flex-1 overflow-hidden">
        <SpreadsheetGrid<ClientInvoice> columns={columns} rows={filtered} onCellChange={handleCellChange} onDeleteRow={onDeleteRow} onAddRow={onAddRow} getRowId={(c) => c.id} emptyMessage="No client invoices yet. Click 'Add Row' or import from Excel." />
      </div>
    </div>
  )
  );
