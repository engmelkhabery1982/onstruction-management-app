...
export function Dashboard({
  ...
  const filteredProjectStats = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter((p) => p.name.toLowerCase().includes(q) || (p.client || '').toLowerCase().includes(q) || (p.location || '').toLowerCase().includes(q) || (p.project_manager || '').toLowerCase().includes(q));
    }
    return items;
